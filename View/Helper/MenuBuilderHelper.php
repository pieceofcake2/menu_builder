...
class MenuBuilderHelper extends AppHelper
{
    ...
    public function __construct(View $View, $settings = [])
    {
        ...
        if (
            isset($View->viewVars[$this->settings['authVar']]) &&
     ...
     */
    public function build($id = null, $options = [], &$data = null, &$isActive = false)
    {
        if ($data === null) {
            $data =& $this->_menu;
        }
    ...
    protected function _buildItem(&$item, $pos = -1, &$isActive = false)
    {
        ...
        if ($hasChildren = is_array($item['children'])) {
            ...
            $children = $this->build('children', [], $item, $nowIsActive);
        ...
        if ($item['url'] === null) {
            ...
            if (is_array($item['url'])) {
                $item['url'] += ['plugin' => false];
                $prefixes = (array)Configure::read('Routing.prefixes');
                foreach ($prefixes as $prefix) {
                    $item['url'] += [$prefix => false];
                }
            }

            $urlOptions = ['title' => trim(html_entity_decode(strip_tags($item['title'])), chr(0xC2) . chr(0xA0))];
            ...
            if (!empty($item['image'])) {
                ...
                $url = $this->Html->link($this->Html->image($item['image'], ['alt' => $item['title']]) . '<span class="label">' . $labelTitle . '</span>', $item['url'], $urlOptions);
            } else {
